...
- **纯手机对传**：主界面可直接选择“接收另一部手机”或“连接另一部手机”，通过局域网自动发现对方，不再需要电脑转发。
- **二维码分享**：只有发送方安装应用也可以传输。选择文件并生成二维码，对方使用任意浏览器扫码即可单独下载，或打包下载 ZIP。
- **复杂网络增强**：自动避开 VPN 对局域网路由的劫持；热点、Wi-Fi Direct、多网卡中单条通道不可用或传输中断时，其余通道可继续工作。
- **断点续传**：浏览器接收支持 HTTP Range，可恢复中断的大文件下载；应用之间（手机↔电脑、手机↔手机）也支持续传，重传时只补传缺失的区块。
- **界面重构**：常用传输方式成为首页直接入口，高级网卡设置下沉到“连接详情”。
- **跨网络分享**：手机具备公网 IPv6 时，二维码分享可以在**不同网络**之间使用，接收方依然只需要浏览器。

二维码分享只在当前网络提供临时下载页，并使用随机访问令牌防止链接被猜中；发送方停止分享后，链接立即失效。浏览器兼容模式使用 HTTP 明文传输，不等同于端到端加密，请只在可信局域网或可信 VPN 中使用，并在接收完成后停止分享。

### 传输中断与续传

接收端**不会**在目标位置留下“大小正常但内容残缺”的文件。数据先写入同目录的 `文件名.hfxpart`，全部区块到齐并校验通过后才改名成正式文件名。因此：

- 传输中断时，目标目录里只会看到 `.hfxpart` 文件，原有的同名文件**不会被破坏**（旧版本会在一开始就把目标文件截断）；
- 同时接收端会在目标目录下写一个 `.hfxresume/` 目录，记录已完成区块和对应文件的 `路径/大小/修改时间`；
- **用同样的选择重传**时，接收端会告诉发送端自己已经有哪些区块，发送端对已有区块只发元数据、不发数据，只补传缺失部分；
- 只有 `路径、大小、修改时间` 全部一致才会复用记录。源文件被改动过就会自动整文件重传，不会拼出错误内容；
- `.hfxpart` 文件被删除或大小不对时，该记录会被直接丢弃（宁可重传，也不冒拼错的风险），所以**请不要手动改动 `.hfxpart` 文件**；
- 传输成功后 `.hfxpart` 与 `.hfxresume` 都会被清理。

`.hfxpart` 和 `.hfxresume` 都是可以随时删除的临时文件，删除的代价只是下次重新传完整文件。

### 跨网络分享（仅 IPv6）

默认情况下二维码分享是一个**局域网**功能。当地址列表里出现**公网 IPv6** 地址时，把该地址生成的二维码发给处在**其他网络**的人也能打开——这是唯一不需要任何中继服务器的跨网络方式。

实测已可用：**家庭移动宽带 ↔ 联通蜂窝流量**之间跨运营商互联成功。但这取决于运营商策略，成立条件缺一不可：

1. 手机的运营商/路由器分配了**公网 IPv6**（地址列表里会标注为公网地址）；
2. 运营商**没有封禁入站连接**（部分移动网络会封禁，这是最常见的失败原因）；
3. 接收方所在网络也支持 IPv6。

也就是说它**并非在所有网络下都能用**，请以实际能否打开为准。应用会在开始分享后自动用所选地址请求一次自己的页面，打不通时会提示“所选地址无响应”，此时换一个地址即可。地址列表里没有公网地址，说明当前网络拿不到公网 IPv6，这条路走不通。

安全提醒（比局域网模式更重要）：跨网络分享走的是**明文 HTTP**，链路上任何一跳都可能看到内容，访问令牌也直接写在链接里——**拿到链接的人在此次分享停止前都能下载**。只发给你信任的人，并在对方接收完成后立即停止分享。不要用这种方式分享敏感文件。
...
- 蓝牙网络共享（不建议用）
- 物理以太网口
- VPN软件的TUN网卡（自动识别并默认关闭；局域网连接会优先绑定到底层网络）
...
本应用可直接完成手机对传，不需要 Termux、ADB 或电脑：

1. 接收方点击“接收另一部手机”，确认要使用的 Wi-Fi、热点或 Wi-Fi Direct 网卡，然后开始等待。
2. 发送方点击“连接另一部手机”。应用会自动搜索附近的接收端；自动发现不可用时仍可手动输入接收方显示的 IP。
3. 连接成功后进入双栏文件界面，选择文件并传输。

`USB_ADB` 现在默认关闭，仅在确实使用电脑进行 ADB 端口转发时手动启用。VPN 和移动网络接口也默认关闭，防止不可达通道拖垮手机直连。

如果对方没有安装应用，发送方点击“二维码分享”，选择文件和对方可访问的 Wi-Fi/热点地址，再让对方用浏览器扫码。浏览器页面不依赖互联网，也不需要安装任何客户端。

> 升级提示：本次传输协议版本为 302（新增续传协商）。应用端与电脑端必须同时升级；旧版会明确提示版本不匹配，而不会继续进行不安全的兼容传输。
>
> 安全提示：手机直连与二维码下载服务仅适合可信的局域网、个人热点或可信 VPN。当前传输协议不提供端到端加密或身份认证；分享期间请勿连接不可信网络，并在接收完成后立即停止服务。
