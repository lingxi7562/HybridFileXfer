...
- **跨网络分享（实验性）**：手机具备公网 IPv6 时，二维码分享可以在**不同网络**之间使用，接收方依然只需要浏览器。
...
2. 运营商**没有封禁入站连接**（移动网络常见封禁，这一条最常导致失败）；
3. 接收方所在网络也支持 IPv6。

因此**它在很多网络下就是不通的**，属于实验性功能，请以实际能否打开为准。应用会在开始分享后自动用所选地址请求一次自己的页面，打不通时会提示“所选地址无响应”，此时换一个地址即可。
